...
SymmetryDESDlg::OnInitCtrl()
{
	...
	GetDlgItem(IDC_EDIT_SYMMETRYDES_INITIALVALUE)->SetWindowText("mxEncrypt");
	((CEdit*)GetDlgItem(IDC_EDIT_SYMMETRYDES_INITIALVALUE))->SetLimitText(0x08);
	GetDlgItem(IDC_EDIT_SYMMETRYDES_CIPHERKEY)->SetWindowText("mengxiangEncryptDecrypt");
...
SymmetryDESDlg::UpdateMixCtrlEnable()
{
	...
	pEdit->SetLimitText(true==b3DES?0x10:0x08);
	CString cstrCipherKey;
	pEdit->GetWindowText(cstrCipherKey);
	pEdit->SetWindowText(cstrCipherKey.Left(true==b3DES?0x10:0x08));
