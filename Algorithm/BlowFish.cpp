...
const unsigned int CBlowFish::scm_auiInitP[18] = { 0x243f6a88, 0x85a308d3,
...
CBlowFish::CBlowFish(unsigned char* ucKey, size_t keysize,
	...
	for (j = 0; j < 4; j++)
		for (int k = 0; k < 256;)
			Encrypt(block), m_auiS[j][k++] = block.m_uil, m_auiS[j][k++]
					= block.m_uir;
}
...
//Returns false if n is multiple of 8
void CBlowFish::Encrypt(const unsigned char* in, unsigned char* out, size_t n,
		int iMode) {
	//Check the buffer's length - should be > 0 and multiple of 8
	if ((n == 0) || (n % 8 != 0))
		return;
...
//Returns false if n is multiple of 8
void CBlowFish::Decrypt(const unsigned char* in, unsigned char* out, size_t n,
		int iMode) {
	//Check the buffer's length - should be > 0 and multiple of 8
	if ((n == 0) || (n % 8 != 0))
		return;
