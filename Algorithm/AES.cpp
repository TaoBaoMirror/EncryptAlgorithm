...
* @(#)
*
* (c)  XIAOTING PRIVATE PERSON  2014 QQ:451292510
* All Rights Reserved. QQ:451292510 Email:xiaoting_chen2010@163.com QQ:451292510 Email:xiaoting_chen2010@163.com
*/

/**
@brief aes（高级加密标准）
    高级加密标准（英语：Advanced Encryption Standard，缩写：AES），在密码学中又称Rijndael加密法，是美国联邦政府
	采用的一种区块加密标准。这个标准用来替代原先的AES，已经被多方分析且广为全世界所使用。经过五年的甄选流程，
...
// AES::AES( AES_KEYSIZE keysize, BYTE *key ):
// Nb(0x00),
// Nk(0x00),
// Nr(0x00),
// key(NULL),
// w(NULL)
// {
// 	memset(Tmp, 0x00, sizeof(char)*256);
// 	memset(tmpA, 0x00, sizeof(bool)*64);
// 	memset(tmpB, 0x00, sizeof(bool)*64);
// 	SetKey(keysize, key);
// }
...
bool 
AES::ECB(char *Out, char *In, long datalen, const char *Key, AES_KEYSIZE keySize, AES_ENCRYPTDECRYPT_TYPE Type)
{
	if(!(Out && In && Key && (datalen=(datalen+15)&0xfffffff0)))
		return false;
	SetKey(keySize, (BYTE*)Key);
	if (AES_ENCRYPTDECRYPT_ENCRYPT == Type) {
		for(long i=0, j=datalen>>4; i<j; ++i, Out+=16, In+=16)
			Cipher((BYTE*)In, (BYTE*)Out);
	}
	else if (AES_ENCRYPTDECRYPT_DECRYPT == Type) {
		for(long i=0, j=datalen>>4; i<j; ++i, Out+=16, In+=16)
			InvCipher((BYTE*)In, (BYTE*)Out);
	}
	else {
		;
	}
...
bool 
AES::CBC(char *IV, char *Out, char *In, long datalen, const char *Key, AES_KEYSIZE keySize, AES_ENCRYPTDECRYPT_TYPE Type)
{
	if(!(Out && In && Key && (datalen=(datalen+15)&0xfffffff0)))
		return false;
	SetKey(keySize, (BYTE*)Key);
	if (AES_ENCRYPTDECRYPT_ENCRYPT == Type) {
		Xor(In, IV, 16);
		for(long i=0, j=datalen>>4; i<j; ++i, Out+=16) {
			Cipher((BYTE*)In, (BYTE*)Out);
			In += 16;
			Xor(In, Out, 16);
		}
...
bool 
AES::CFB(char *IV, char *Out, char *In, long datalen, const char *Key, AES_KEYSIZE keySize, AES_ENCRYPTDECRYPT_TYPE Type)
{
	if(!(Out && In && Key))
		return false;
	SetKey(keySize, (BYTE*)Key);
	for(long i=0; i<datalen; ++i, ++Out, ++In) {
		Cipher((BYTE*)IV, (BYTE*)Out);
		Xor(Out, In, 1);
		RotateL(IV, 16, 1);
		AES_ENCRYPTDECRYPT_ENCRYPT==Type?(IV[15] = Out[0]):(IV[15] = In[0]);
	}
	return true;
}

bool 
AES::OFB(char *IV, char *Out, char *In, long datalen, const char *Key, AES_KEYSIZE keySize, AES_ENCRYPTDECRYPT_TYPE Type)
{
	if(!(Out && In && Key))
		return false;
	SetKey(keySize, (BYTE*)Key);
	char t = 0x00;
	for(long i=0; i<datalen; ++i, ++Out, ++In) {
		Cipher((BYTE*)IV, (BYTE*)Out);//注意使用的是加密函数!
		t = Out[0];
		Xor(Out, In, 1);
		RotateL(IV, 16, 1);
		IV[15] = t;
...
bool 
AES::CTR(char *CNT, char *Out, char *In, long datalen, const char *Key, AES_KEYSIZE keySize, AES_ENCRYPTDECRYPT_TYPE Type)
{
	long i, j, count; // 千万不能定义成long cnt;
	if(!(Out && In && Key && (datalen=(datalen+15)&0xfffffff0)))
		return false;
	SetKey(keySize, (BYTE*)Key);
	for(i=0, j=datalen>>4; i<j; ++i, Out+=16, In+=16) {
		Cipher((BYTE*)CNT, (BYTE*)Out);//注意使用的是加密函数!
		Xor(Out, In, 16);
		count = atol(CNT);
		ltoa(count, CNT, 128);
	}
...
void 
AES::SetKey(AES_KEYSIZE keysize, BYTE *key)
{
	this->Nb = 4;
	switch( keysize )
	{
	case AES::AES_KEYSIZE_BIT128:
		this->Nk = 4;
		this->Nr = 10;
		break;
	case AES::AES_KEYSIZE_BIT192:
		this->Nk = 6;
		this->Nr = 12;
		break;
	case AES::AES_KEYSIZE_BIT256:
	default:
		this->Nk = 8;
		this->Nr = 14;
		break;
...
void AES::Xor(bool *InA, const bool *InB, int len)
{
	for(int i=0; i<len; ++i)
		InA[i] ^= InB[i];
}
void AES::Xor(char *InA, const char *InB, int len)
{
	//	bool tmpA[8*len], tmpB[8*len];
	ByteToBit(tmpA, InA, 8*len);
	ByteToBit(tmpB, InB, 8*len);
	Xor(tmpA, tmpB, 8*len);
	BitToByte(InA, tmpA, 8*len);
}
...
void AES::RotateL(char *In, int len, int loop)
{
	//	bool tmp[8*len];
	ByteToBit(tmpA, In, 8*len);
	RotateL(tmpA, 8*len, 8*loop);
